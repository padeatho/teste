...
```mermaid
graph TD
    PRE[tPrejob: conexão DB + setup] --> CLN[tFileList -> tFileDelete (limpeza *.zip/*.csv)]
    CLN --> DWN[tFileFetch (download ZIP)]
    DWN --> UNZ[tFileUnarchive (extração)]
    UNZ --> S1[proponentes]
    UNZ --> S2[convenio]
    UNZ --> S3[empenho]
    UNZ --> S4[desembolso]
    UNZ --> S5[cronofisico]
    UNZ --> S6[termo_aditivo]
    UNZ --> S7[solicitacao_rendimento_aplicacao]
    UNZ --> S8[contraproposta]
    S1 --> POST[tPostjob: close + métricas + logs]
    S2 --> POST
    S3 --> POST
    S4 --> POST
    S5 --> POST
...
```mermaid
flowchart TD
    A[Falha em Subjob] --> B{Tipo de Erro?}
    B -->|Rede/Download| C[Retry manual do download ou reexecução geral]
    B -->|CSV inválido| D[Isolar arquivo e registrar detalhes]
    B -->|Regra de dado| E[Enviar para DLQ (arquivo rejeitados) + corrigir regra]
    C --> Z[Relançar job]
    D --> Z
    E --> Z
```
